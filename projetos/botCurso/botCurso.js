...
bot.hears('Como comprar o curso?',  ctx=>{
     ctx.replyWithMarkdown('Que bom... [link](http://www.google,com/)',tecladoOpcoes)
})
bot.action('n',ctx =>{
...
bot.hears(/mensagem qualquer/i,  ctx=>{
     ctx.replyWithMarkdown('Essa piada é velha tente outra...',tecladoOpcoes)
})
...
bot.on('location',async ctx=>{
    try{
        ...
        //console.log(lat,lon)
        const res = await axios.get(`${url}?lat=${lat}&lon=${lon}&APPID=9ca10e970b16608197de37d5908217f4&units=metric`)
        await ctx.reply(`Huum... Você está em ${res.data.name}`)
        await ctx.reply(`A temperatura por ai está em ${res.data.main.temp}°C`,tecladoOpcoes)
    }catch(e){
      ctx.reply('Estou tendo problemas para pegar sua localização, você está no planeta terra? :P',tecladoOpcoes)
    }
})
